    ...
    grad = jax.tree_map(
        lambda g: g / jnp.sqrt(
            sum([
                ...
                for p in jax.tree_util.tree_leaves(grad)
            ])
        ), grad)
    noisy_params = jax.tree_multimap(lambda p, g: p + args.sam_rho * g, state.params, grad)

    # batch_stats and metrics should be computed only for the first one
    metrics, new_model_state = aux

    # get gradients at the noisy parameters
    (_, _), grad = grad_fn(noisy_params)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--sam_rho', default=0.05, required=True, type=float)
